...
class p5Tanks extends p5 {
  ...
  constructor(
    arg0: (...args: any[]) => any,
    arg1?: string | HTMLElement | undefined
  ) {
    super(arg0, arg1);
  ...
  public Tank = class {
    ...
    constructor(
      ...
      if (args.length > parameterKeys.length) {
        throw new Error(
          `Too many arguments for p5Tanks.Tank constructor.\nNumber of arguments: ${
            args.length
      ...
      this._dispersion =
        this.p.atan(
          parameterInitializers.shellMass / parameterInitializers.barrelLength
        ) / p5Tanks.DISTANCE_SCALAR;
      this._maxSpeed = parameterInitializers.maxSpeed * p5Tanks.SPEED_SCALAR; // 17.78 [m/s]

      //p5play members
      this._turretAssembly = new this._modules.Group();

      this._hull = new this._modules.Sprite(
        X,
        Y,
        parameterInitializers.width * p5Tanks.DISTANCE_SCALAR,
        parameterInitializers.length * p5Tanks.DISTANCE_SCALAR,
        "d"
      );
      this._tracks = {
        t0: new this._modules.Sprite(
          X,
          Y,
          parameterInitializers.wheelWidth * p5Tanks.DISTANCE_SCALAR,
          parameterInitializers.length * p5Tanks.DISTANCE_SCALAR,
          "d"
        ),
        t1: new this._modules.Sprite(
          X,
          Y,
          parameterInitializers.wheelWidth * p5Tanks.DISTANCE_SCALAR,
          parameterInitializers.length * p5Tanks.DISTANCE_SCALAR,
      ...
      this._tracks.t0.offset.x =
        this._hull.halfWidth +
        (parameterInitializers.wheelWidth * p5Tanks.DISTANCE_SCALAR) / 2;
      this._tracks.t1.offset.x =
        -this._hull.halfWidth -
        (parameterInitializers.wheelWidth * p5Tanks.DISTANCE_SCALAR) / 2;
      this._turret = new this._turretAssembly.Sprite(
        X,
        Y + this._hull.halfHeight - this._hull.height / 3,
        this._hull.width
      );
      let calibre =
        Math.sqrt(parameterInitializers.shellMass / Math.PI) * 0.0306 * 2;
      this._gun = new this._turretAssembly.Sprite(
        this._turret.x,
        this._turret.y +
          (parameterInitializers.barrelLength * p5Tanks.DISTANCE_SCALAR) / 2 +
          (this._hull.halfWidth * 2) / 3,
        (calibre + 0.08) * p5Tanks.DISTANCE_SCALAR,
        parameterInitializers.barrelLength * p5Tanks.DISTANCE_SCALAR,
        "none"
      );
      ...
      this._hull.mass = this._mass * p5Tanks.DISTANCE_SCALAR * 0.6;
      this._tracks.t0.mass = this._mass * p5Tanks.DISTANCE_SCALAR * 0.2;
    ...
    steer(power: Tank.Direction): void {
      ...
      if (
        this.speed > this.p.world.velocityThreshold &&
    ...
    turnTurret(
      power: Tank.Direction | number | { x: number; y: number }
    ): Tank.Direction {
      ...
      if (typeof power != "number") {
        throw new Error(
          `power imput of Tank.turnTurret() is '${typeof power}'. Input must be Tank.Direction, number, or {x: number, y: number}`
        );
      }
      const SPEED_AMPLITUDE: number =
        (power * p5Tanks.SPEED_SCALAR * this._maxSpeed) / this._mass;
      this._turretAssembly.rotationSpeed =
    ...
    get speed(): number {
      return p5Tanks.prototype.dist(
        0,
        0,
        this._hull.velocity.x,
        this._hull.velocity.y
      );
    }
    get trackSpeed(): { s0: number; s1: number } {
      return {
        s0: p5Tanks.prototype.dist(
          0,
          0,
          this._tracks.t0.velocity.x,
          this._tracks.t0.velocity.y
        ),
        s1: p5Tanks.prototype.dist(
          0,
          0,
          this._tracks.t1.velocity.x,
          this._tracks.t1.velocity.y
        ),
      };
    }
    get maxSpeed(): number {
    ...
    get dimensions(): object {
      return {
        hull: {
          width: this._hull.w / p5Tanks.DISTANCE_SCALAR,
          length: this._hull.h / p5Tanks.DISTANCE_SCALAR,
        },
        tracks: { width: this._tracks.t0.w / p5Tanks.DISTANCE_SCALAR },
        turret: { radius: this._turret.r / p5Tanks.DISTANCE_SCALAR },
        gun: {
          width: this._gun.w / p5Tanks.DISTANCE_SCALAR,
          length: this._gun.h / p5Tanks.DISTANCE_SCALAR,
        },
      };
    }
    ...
    get motionDirection(): number {
      p5Tanks.prototype.angleMode(p5Tanks.prototype.DEGREES);
      return this.normalizeTo180(
        p5Tanks.prototype.atan2(this.velocity.y, this.velocity.x)
      );
    }
    ...
    getPerpendicularDistance2Turret(x: number, y: number): number {
      const DISTANCE = p5Tanks.prototype.dist(
        x,
        y,
        this._turret.x,
        this._turret.y
    ...
    decideTurretTurningDirection(
      x: number,
      y: number,
      threshold: number = (this.dispersion * 2) / 3
    ): Tank.Direction {
      p5Tanks.prototype.angleMode(p5Tanks.prototype.DEGREES);
      if (threshold < 0) {
        throw new Error(
          "The input threshold of decideTurretTurningDirection cannot be a negative number"
        );
      }
...
p5Tanks.prototype.registerMethod("pre", function applySideDragForce() {
  for (const P of p5Tanks.INSTANCES) {
    for (const TANK of P.TANKS) {
      ...
      const FORCE_MAGNITUDE: number =
        Math.abs(P.sin(TANK.motionDirection - TANK.direction)) *
        TANK.speed *
        5000;
